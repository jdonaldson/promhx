...
var ErrorOrder = function() { };
ErrorOrder.__name__ = true;
ErrorOrder.main = function() {
	var d1 = new promhx_Deferred();
	var d2 = new promhx_Deferred();
	var d3 = new promhx_Deferred();
	var p1 = d1.promise();
	var p2 = d2.promise();
	var p3 = d3.promise();
	promhx_Promise.whenAll([p1,p2,p3]).then(function(r) {
		console.log("complete " + Std.string(r));
...
promhx_base_AsyncBase.prototype = {
	...
	,_resolve: function(val) {
		...
		if(this._pending || this._errorPending) promhx_base_EventLoop.enqueue((function(f,a1) {
			return function() {
				f(a1);
			};
	...
	,link: function(f,next) {
		this._update.push({ async : next, linkf : function(x) {
			next.handleResolve(f(x));
		}});
		promhx_base_AsyncBase.immediateLinkUpdate(this,next,f);
	}
	,pipeLink: function(f,ret) {
		var linked = false;
		...
		this._update.push({ async : ret, linkf : linkf});
		if(this._resolved && !this._pending) try {
			linkf(this._val);
		} catch( e ) {
			if (e instanceof js__$Boot_HaxeError) e = e.val;
			ret.handleError(e);
...
promhx_Stream.prototype = $extend(promhx_base_AsyncBase.prototype,{
	...
	,handleEnd: function() {
		if(this._pending) promhx_base_EventLoop.enqueue($bind(this,this.handleEnd)); else if(this._end_promise._resolved) return; else {
			this._end = true;
			var o;
...
;
	promhx_base_EventLoop.nextLoop = rawAsap;
} else promhx_base_EventLoop.nextLoop = function(x) {
	haxe_Timer.delay(x,0);
};
promhx_base_EventLoop.queue = new List();
ErrorOrder.main();
